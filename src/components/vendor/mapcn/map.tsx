...
/**
 * Vendored from mapcn (https://github.com/AnmolSaini16/mapcn, MIT), registry item
 * https://mapcn.dev/r/map.json. Copied in rather than installed, as the design system does
 * with shadcn components. Local changes, and nothing else:
 *   - `cn` comes from @upcytech/ui;
 *   - the MapLibre worker is self-hosted (scripts/copy-maplibre-worker.ts), never unpkg;
 *   - shadcn theme classes are mapped to the design system's tokens.
 * The CARTO basemap styles are unchanged; the page only loads this module after the visitor
 * asks for the map (components/contact/office-map.tsx).
 */
...
function DefaultLoader() {
  return (
    <div className="bg-canvas/50 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
      <div className="flex gap-1">
        <span className="bg-muted/60 size-1.5 animate-pulse rounded-full" />
        <span className="bg-muted/60 size-1.5 animate-pulse rounded-full [animation-delay:150ms]" />
        <span className="bg-muted/60 size-1.5 animate-pulse rounded-full [animation-delay:300ms]" />
      </div>
    </div>
  );
}
...
function PopupCloseButton({ onClick, label = "Close popup" }: { onClick: () => void; label?: string }) {
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className="focus-visible:ring-ring hover:bg-raised text-ink absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset"
    >
...
type MarkerPopupProps = {
  ...
  closeButton?: boolean;
  /** Accessible name for the popup close button (default: "Close popup") */
  closeLabel?: string;
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-raised text-ink relative max-w-62 rounded-md border p-3 shadow-md",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
      {closeButton && <PopupCloseButton onClick={handleClose} label={closeLabel} />}
      {children}
    </div>,
    container,
  );
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-ink text-canvas pointer-events-none rounded-md px-2 py-1 text-xs text-balance shadow-md",
        "animate-in fade-in-0 zoom-in-95 duration-200 ease-out",
        className,
      )}
    >
...
type MapControlsProps = {
  ...
  /** Accessible names for the control buttons. English defaults preserve upstream behaviour. */
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-hairline bg-canvas [&>button:not(:last-child)]:border-hairline flex flex-col overflow-hidden rounded-md border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex size-8 items-center justify-center transition-colors pointer-coarse:size-11",
        "first:rounded-t-md last:rounded-b-md",
        "hover:bg-raised dark:hover:bg-raised/40",
...
function MapControls({
  ...
  return (
    ...
    >
      {showZoom && (
        <ControlGroup>
          <ControlButton onClick={handleZoomIn} label={t.zoomIn}>
            <Plus className="size-4" />
          </ControlButton>
          <ControlButton onClick={handleZoomOut} label={t.zoomOut}>
      ...
      {showCompass && (
        <ControlGroup>
          <CompassButton onClick={handleResetBearing} label={t.resetBearing} />
        </ControlGroup>
      )}
      {showLocate && (
        <ControlGroup>
      ...
      {showFullscreen && (
        <ControlGroup>
          <ControlButton onClick={handleFullscreen} label={t.fullscreen}>
  ...
  return (
    <ControlButton onClick={onClick} label={label}>
      ...
      >
        ...
        <path d="M12 22L16 12H12V22Z" className="fill-muted/60" />
        <path d="M12 22L8 12H12V22Z" className="fill-muted/30" />
      </svg>
    </ControlButton>
  );
}
